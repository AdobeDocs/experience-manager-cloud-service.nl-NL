...
* `errors` de expressie vermelden van de velden waarvoor de validatiecriteria niet zijn nageleefd, samen met het foutbericht voor de validatie.
...
Voeg de volgende code aan het dossier van JavaScript toe om de reactie en kopballen te tonen, die van het de diensteindpunt van REST, in de browser console worden ontvangen.
   ...
   >[!NOTE]
   >
   > In de `.content.xml` bestand, toevoegen `categories = [custom-errorhandler-name]`. In dit geval bijvoorbeeld: [custom-errorhandler-name] wordt verstrekt zoals `customfunctionsdemoV2`.
...
![De naam van de clientbibliotheek toevoegen in de configuratie van de adaptieve formuliercontainer](/help/forms/assets/client-library-category-name-core-component.png)
...
![Aangepaste fouthandler toevoegen aan een formulier voor het verwerken van reacties op fouten](/help/forms/assets/custom-error-handler.png)s


Als resultaat van deze regel worden de waarden waarvoor u invoert **Huisdier-id** controleert validatie voor **Naam huisdier** het gebruiken van externe dienst die door het eindpunt van REST wordt aangehaald. Als de validatiecriteria op basis van de gegevensbron mislukken, worden de foutberichten weergegeven op veldniveau.

![Voeg een aangepaste fouthandler toe aan een formulier voor het verwerken van reacties op fouten](/help/forms/assets/custom-error-handler-message-core-component.png)
