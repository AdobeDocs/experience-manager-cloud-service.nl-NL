...
* **Sjabloon het Vergrendelen**: Vergrendel inhoud en lay-outelementen binnen malplaatjes om merkintegriteit te handhaven en onbevoegde wijzigingen te verhinderen.

* **Overloop van de Inhoud**: &quot;sta pagina-einden binnen inhoud&quot;optie voor stroomde lay-outs voor vlotte multi-page het uitgeven en beter tekstbeheer voor complexe documenten toe.

* **XDP het Uitgeven van het Dossier**: U kunt XDP dossiers in browser in plaats van Forms Designer nu uitgeven die slechts op de Desktop van Microsoft Windows loopt.

* **Roep Geassocieerde UI op een Publish instantie** aan: U kunt Associate UI op Publish instanties nu direct aanhalen. Dit vermogen bepaalt de vereiste configuratie, de loonlaststructuur, en aanroepingsstroom, die integratie vereenvoudigen en plaatsing over milieu&#39;s versnellen.
...
* [ Directe API integratie ](/help/forms/api-integration-in-rule-editor.md): De Visuele Redacteur van de Regel in Aanpassings Forms steunt nu directe API integratie zonder een Model van de Gegevens van de Vorm te vereisen. U kunt verbinding maken met een API-eindpunt door de JSON-URL in te voeren of door de configuratie te importeren met een cURL-opdracht. Na integratie kan de API worden aangeroepen met de handeling `Invoke Service` .

* [ Verbeterde Navigatie met de Steun van de Payload van de Gebeurtenis ](/help/forms/invoke-service-enhancements-rule-editor.md#use-case-5-use-event-payload-in-navigate-to-action-in-invoke-service): **navigeer aan** actie in de Invoke de handlers van de Dienst `EVENT_PAYLOAD`, toestaand vormauteurs om follow-upacties te vormen die op gebeurtenisreacties worden gebaseerd.

* [ Functie en de Wiskundige Steun van de Uitdrukking in de Parameters van de Input ](/help/forms/rule-editor-core-components-user-interface.md#function-and-mathematical-expression-support-in-input-parameters): De parameters van de input steunen nu zowel functievraag als wiskundige uitdrukkingen, toelatend vormauteurs om dynamisch gegevens verwerkte waarden direct over te gaan.

* [ wint bezitswaarden van een serie JSON ](/help/forms/invoke-service-enhancements-rule-editor.md#retrieve-property-values-from-a-json-array) terug: Oproep API met een douanefunctie om waarden uit een serie te halen JSON en hen direct te binden aan vormgebieden.

### Ondersteuning voor versies in Forms Manager

Forms Manager ondersteunt nu versioning voor Adaptive Forms (Core Components and Foundation Components), formulierfragmenten, thema&#39;s, XDP-sjablonen en binaire elementen. U kunt versies maken, de historie bekijken en eerdere staten herstellen via de Forms &amp; Documents-console. Zie [ vormversies in de Manager van Forms beheren ](/help/forms/manage-form-versions-forms-manager.md).

### Verbeterde toegankelijkheid voor formuliercomponenten

Aangepaste Forms Core-componenten introduceren semantische opmaakcodes die voldoen aan WCAG voor groepen selectievakjes, groepen keuzerondjes en deelvensters. Deze componenten gebruiken nu `<fieldset>` en `<legend>` -elementen om zinvolle relaties tot stand te brengen tussen groepslabels en hun opties voor ondersteunende hulpmiddelen. Zie [ de steun van het Gebied en van de legenda in Aanpassings Forms ](/help/forms/creating-accessible-adaptive-forms.md#fieldset-legend-accessibility).
