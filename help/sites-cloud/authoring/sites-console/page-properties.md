...
* **Login Pagina** - de pagina die voor login moet worden gebruikt
  ...
  >[!WARNING]
  >
  >De login gespecificeerde pagina **moet niet** richten aan een pagina die zelf door een **Vereiste van de Authentificatie** wordt beschermd. Zorg ervoor dat de aanmeldingspagina een openbare, niet-beveiligde pagina is. Bijvoorbeeld, een specifieke pagina zoals `/content/<mysite>/en/login` zonder **toegelaten Vereiste van de Authentificatie**.
  >
  >Als de gevormde login pagina **heeft laten** controleren het authentificatievereiste op die pagina stil bij opstarten wordt verwijderd, die het openbaar toegankelijk maken.
