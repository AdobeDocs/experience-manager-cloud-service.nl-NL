...
De releasedatum van [!DNL Adobe Experience Manager] als een [!DNL Cloud Service] huidige release met functies (2026.2.0) is 3 maart 2026. De volgende release met functies (2026.3.0) is gepland voor 26 maart 2026.
...
Bekijk de video Overzicht van de release van februari 2026 voor een overzicht van de functies die in de release van 2026.2.0 zijn toegevoegd:

>[!VIDEO](https://video.tv.adobe.com/v/3480405/?captions=dut&quality=12)
...
**Op attributen-gebaseerde toegangscontrole (ABAC) voor Dynamische Media met OpenAPI**

Op attributen-gebaseerde toegangsbeheer (ABAC) laat beheerders toe om toegang tot Dynamische Media met activa te controleren OpenAPI gebruikend meta-gedreven regels. Beheerders kunnen regels definiëren voor gebruikersgroepen op basis van metagegevens van elementen om te bepalen welke elementen zichtbaar zijn voor specifieke groepen. Wanneer de metagegevens van een element overeenkomen met de gedefinieerde voorwaarden, wordt automatisch toegang verleend. Met deze functionaliteit kunnen organisaties beter bestuur afdwingen, zodat gebruikers alleen met Dynamic Media kunnen werken met OpenAPI-middelen die relevant zijn voor hun rol of machtigingen.
...
#### Het Oplossen van problemen van de Pijpleiding van de Kwaliteit van de code met de Agent van de Ontwikkeling {#devagent-codequality}

De mogelijkheden van de het oplossen van problemenoplossing van de pijpleiding van de Agent van de Ontwikkeling helpen ontwikkelaars efficiënter problemen in de plaatsingen van AEM as a Cloud Service diagnostiseren en oplossen.

Eerder geconcentreerd op de **Bouwstijl &amp; de Testen van de Eenheid** stap, steunt het pijpleidingsoplossen van problemen nu ook de **Scannen van de Code** stap in Volledige Plaatsing van de Stapel en de pijpleidingen van de Kwaliteit van de Code.
...
Met AEM Edge-functies kunt u JavaScript uitvoeren op de CDN-laag, waardoor de gegevensverwerking dichter bij de eindgebruiker komt te staan. Dit vermindert latentie en maakt responsieve, dynamische ervaringen aan de rand mogelijk.
...
>[!VIDEO](https://video.tv.adobe.com/v/3480348/?captions=dut&quality=12)

Moderne IDEs gebruikt het ModelProtocol van de Context (MCP) om grote taalmodellen (LLMs) toe te laten om hulpmiddelen aan te halen die door servers MCP worden blootgesteld. In plaats van rechtstreeks te integreren met API-specificaties op laag niveau, kunnen ontwikkelaars hun intentie eenvoudig in natuurlijke taal beschrijven.
