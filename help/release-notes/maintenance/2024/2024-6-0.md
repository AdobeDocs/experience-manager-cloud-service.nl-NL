...
1. Als een gebruiker de AEM Forms SDK-versie van meer dan `AEM Forms add-on v2024.05.04.00-240400` downloadt, kan de Docker-service niet worden gestart in het batchbestand. U lost dit probleem als volgt op:
   ...
   1. Vervang de bestaande `sdk.sh` - en `sdk.bat` -bestanden in de SDK van AEM Forms door de nieuwe bestanden.
...
* HULPLIJNEN-17088: De functionaliteit om de documentstaat van het **paneel van de Eigenschappen van het Dossier te veranderen** werkt correct niet en verandert in de *3} staat van het Ontwerp {.*
