...
| `envTypes` | Deze array van tekenreeksen is een onderliggende eigenschap van het knooppunt `metadata` . Voor **publiceer Levering**, zijn de mogelijke waarden dev, stadium, prod, of om het even welke combinatie, en het bepaalt voor welke milieutypes de configuratie wordt verwerkt. Als de array bijvoorbeeld alleen `dev` bevat, wordt de configuratie niet geladen in werkgebied- of prodomgevingen, zelfs niet als de configuratie daar wordt geïmplementeerd. Voor **Edge Delivery**, slechts zou een waarde van `prod` moeten worden gebruikt. | Alle omgevingstypen, d.w.z. (dev, stage, prod) voor levering publiceren of alleen-prod voor Edge Delivery. |
