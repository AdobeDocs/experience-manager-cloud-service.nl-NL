...
|--- |--- |--- |--- |--- |
| `ref` | FragmentSelectorRef | Nee | | Verwijzing naar de instantie `ContentFragmentSelector` , waarmee toegang wordt verleend tot beschikbare functionaliteit zoals `reload` . |
...
| `allowedRepositoryIds` | string[] | Nee | | Lijst met gegevensopslagruimte-id&#39;s voor het filteren van opslagruimten en inhoudsfragmenten in de Content Fragment Selector. Als deze opslagruimte-id&#39;s worden meegeleverd, zijn alleen deze opslagplaatsen zichtbaar in de opslagplaats-kiezer. Als er geen array of lege array is opgegeven, zijn alle opslagruimten waartoe de gebruiker toegang heeft, beschikbaar. |
...
| `getImsToken` | Haalt de verificatie `imsToken` op voor de momenteel aangemelde gebruiker, die kan worden gebruikt voor het verifiëren van aanvragen voor andere services, zoals het genereren van elementuitvoering. |
