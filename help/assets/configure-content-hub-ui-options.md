...
1. Geef een label voor de eigenschap op, wijs deze toe aan een eigenschap met behulp van het veld **[!UICONTROL Metadata]** en selecteer het juiste invoertype voor de nieuwe metagegevens van het element. De invoertypelabels worden bijgewerkt zodat deze worden uitgelijnd met Adobe Experience Manager (AEM). Tot de beschikbare invoertypen behoren Tekst, Codes, Datum, Nummer, Vervolgkeuzelijst, Meerdere waardetekst, Slimme tags en Slimme kleurcodes. Voor bepaalde meta-gegevensgebieden (xcm :colorDistribution, xcm :machineKeywords), wordt het inputtype automatisch toegewezen en kan niet worden gewijzigd.
...
![&#x200B; UI van de Configuratie uploadt details op Content Hub &#x200B;](assets/filter-new.png)
...
1. Geef een label voor het filter op, wijs het filter toe aan een eigenschap met behulp van het veld **[!UICONTROL Metadata]** en selecteer het juiste invoertype voor het nieuwe filter. De invoertypelabels worden bijgewerkt zodat deze worden uitgelijnd met Adobe Experience Manager (AEM). Tot de beschikbare invoertypen behoren Tekst, Codes, Datum, Nummer, Vervolgkeuzelijst, Meerdere waardetekst, Slimme tags en Slimme kleurcodes. Voor bepaalde meta-gegevensgebieden (xcm :colorDistribution, xcm :machineKeywords), wordt het inputtype automatisch toegewezen en kan niet worden gewijzigd.
...
![&#x200B; filters UI van de Configuratie op Content Hub &#x200B;](assets/add-filters.png)
...
1. Schakel **[!UICONTROL Bulk Search]** in. De standaardscheidingstekens `[ , | \t | \r\n | \r | \n ]` worden automatisch weergegeven. Bovendien kunt u ook extra scheidingstekens toevoegen. Hiervoor geeft u scheidingstekens op in het invoervak, gescheiden door een `pipe symbol (|)` .
   ...
   ![&#x200B; BulkConfiguratie van het Onderzoek &#x200B;](assets/edit-filter-new.png)
...
![&#x200B; de activadetails van de Configuratie UI op Content Hub &#x200B;](assets/asset-detailsn.png)
...
### Wie heeft toegang tot de instellingen van de AEM Assets Content Hub-gebruikersinterface en kan deze configureren?

Alleen gebruikers met Content Hub-beheerdersrechten hebben toegang tot de Content Hub-instellingen voor de gebruikersinterface en kunnen deze configureren. De administratieve toestemmingen moeten via Adobe Admin Console worden verleend alvorens tot de configuratieinterface toegang te hebben.

### Wat kunnen beheerders configureren in de gebruikersinterface van AEM Assets Content Hub?

Beheerders kunnen de volgende instelling configureren in de AEM Assets Content Hub Configuration User Interface:

* Metagegevensvelden importeren (inclusief vereiste velden en instellingen voor automatische goedkeuring)

* Filters zoeken die op de zoekpagina worden weergegeven

* Doorzoekbare metagegevensvelden die worden gebruikt voor indexering

* Metagegevens van de Asset Card (maximaal zes velden op kaarten)

* Eigenschappen van de weergave Elementdetails

* Brandingselementen (logo, bannertekst, kleuren, bannerafbeelding)

* Zichtbaarheidsregels voor middelen (inclusief verlopen gedrag van elementen)

* Vertonings- en downloadopties

* Aangepaste navigatiekoppelingen (externe tabbladen)

* Instellingen voor verzamelingen en delen (openbare koppelingen en machtigingen)

Deze instellingen bepalen zowel de visuele ervaring als het functionele gedrag van Content Hub voor eindgebruikers.


### Hoe kunnen beheerders nieuwe metagegevensvelden toevoegen voor het uploaden van elementen in AEM Assets Content Hub?

De beheerders kunnen nieuwe meta-gegevensgebieden voor activa toevoegen uploadt door **te klikken voegt meta-gegevens** in het **3&rbrace; lusje van de Invoer toe, die het specificeren aan het aangewezen bezit van het meta-gegevensschema in kaart brengen, het inputtype selecteren, en het beslissen als het gebied wordt vereist of facultatief.** Na bevestiging en het bewaren, wordt het nieuwe gebied beschikbaar voor gebruikers tijdens activa uploadt.

### Wat is de instelling voor automatische goedkeuring voor elementen in AEM Assets Content Hub en hoe werkt deze?

Met de instelling Automatische goedkeuring kunt u, indien deze is ingeschakeld, nieuwe elementen direct beschikbaar maken op Content Hub tijdens het uploaden. Deze instelling is standaard uitgeschakeld. AEM-auteurs of -beheerders moeten nieuwe middelen handmatig goedkeuren voordat ze op de Content Hub-portal worden weergegeven.

### Hoe configureren beheerders filters voor middelenzoekopdrachten in AEM Assets Content Hub?

De beheerders kunnen filters voor activa onderzoeken vormen door **te klikken voegt filters** toe, die het filteretiket specificeren, het in kaart brengen aan het aangewezen meta-gegevensbezit, het gegevenstype selecteren, en het bevestigen van de toevoeging. Filters worden beschikbaar in de interface als er minstens één element is dat aan de filtercriteria voldoet. De veranderingen worden toegepast door **te klikken sparen**.

### Welke activa eigenschappen kunnen in de mening van de Details van Activa worden getoond en hoe worden zij gevormd in AEM Assets Content Hub?

Elementeigenschappen zoals bestandsnaam, titel, indeling, grootte en aangepaste velden zoals categorieën kunnen worden weergegeven in de weergave Asset Details. De beheerders voegen deze eigenschappen toe door **te klikken voegen meta-gegevens** toe, specificerend het etiket, in kaart brengend het aan het meta-gegevensschema, selecterend het gegevenstype, bevestigend, en het bewaren van de veranderingen.

### Hoe kunnen beheerders de metagegevensdetails aanpassen die worden weergegeven op de assetkaarten in AEM Assets Content Hub?

De beheerders kunnen meta-gegevensdetails aanpassen die op activakaarten worden getoond door **te klikken voegt meta-gegevens** in de montages van de Kaart van Activa toe, die het etiket (bijvoorbeeld, dossierformaat) specificeren, het in kaart brengen aan het aangewezen bezit in het meta-gegevensschema, en het bevestigen van de toevoeging. U kunt maximaal zes metagegevensvelden weergeven op asset cards.

### Kunnen beheerders uw AEM Assets Content Hub-portal aanpassen aan de vereisten van uw organisatie voor het merken van producten?

Ja. Beheerders kunnen de vormgeving van AEM Assets Content Hub aanpassen door de titel en de platte tekst voor de hoofdbanner te bewerken, tekstkleuren te selecteren via een kleurkiezer of hexadecimale code en secundaire en primaire kleuren in te stellen voor achtergrond, tekst en pictogrammen.
