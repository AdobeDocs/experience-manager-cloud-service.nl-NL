...
* Structuurafspeelgebeurtenissen voor programmacode uitzenden
* Ondersteuning van integratie met externe analysemogelijkheden en externe systemen

De viewer is beschikbaar als een extra optie en vereist expliciete selectie wanneer deze wordt ondersteund. Bestaande videoviewers worden niet automatisch vervangen.
...
2. De video kan van de pagina van activadetails worden voorvertoond gebruikend **Video (nieuw)**.
3. De Nieuwe VideoKijker kan in de **Dynamische component van Media** worden geselecteerd wanneer het ontwerpen van de pagina&#39;s van Plaatsen.
...
| Selectie van viewer | Moet expliciet worden geselecteerd |
...
Als uw video&#39;s worden benaderd via directe URL&#39;s in plaats van via de component Dynamische media, kunt u deze overschakelen naar de Nieuwe Video Viewer door de URL bij te werken. Bijvoorbeeld: `https://s7d1.scene7.com/dmviewers/html5/VideoViewer.html?asset=<video-asset>`

## Viewer-opties {#viewer-modifiers}

Met Viewer-opties kunt u het laden van elementen, het afspelen, het selecteren van streaming indelingen en de viewerpresentatie beheren.
...
| `contenturl` | Hiermee geeft u het hoofdpad van de inhoud op. |
| `videoserverurl` | Geeft het hoofdpad van de videoserver op. |
| `sources.dash` | Specificeert DASH manifest URL voor playback. |
| `sources.hls` | Hiermee wordt de HLS-manifest-URL voor afspelen opgegeven. |
...
De gebeurtenislading omvat informatie zoals het gebeurtenistype, playbackstaat, huidige playbacktijd, en extra meta-gegevens. Deze gebeurtenissen kunnen worden gebruikt ter ondersteuning van analyses bijhouden, aangepaste interacties of integratie met externe systemen

Adobe raadt aan de oorsprong van het bericht te valideren om ervoor te zorgen dat gebeurtenissen alleen worden verwerkt vanuit vertrouwde dynamische mediadomeinen.
...
Het rapport bevat maatstaven voor de betrokkenheid per video, zoals weergaven, indrukken, controletijd, voltooiingsscore en betrokkenheidsscore.

### Rapportindeling

* Rapporten worden geleverd in CSV-indeling.
* Elke rij vertegenwoordigt één enkele video.
...
| `video_watched_seconds` | Het totaal aantal seconden dat wordt gecontroleerd tijdens alle afspeelgebeurtenissen. | Som van seconden die worden gecontroleerd tijdens alle afspeelgebeurtenissen |
| `play_rate` | Het percentage videoafspelen in verhouding tot het laden van video. | (`video_views`:1 `video_impressions`) × 100 |
| `avg_time_watched_in_seconds` | Gemiddeld aantal seconden dat per weergave wordt gecontroleerd. | `video_watched_seconds` Ht `video_views` |
