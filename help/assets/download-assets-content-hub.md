...
description: Leer hoe u een of meer middelen en de bijbehorende uitvoeringen downloadt van de Content Hub-portal.
role: User
...
* Meerdere elementen en de bijbehorende uitvoeringen downloaden:
   ...
   1. Optioneel: Hef de selectie van **[!UICONTROL Create a separate folder for each asset]** op om de geselecteerde elementen en hun uitvoeringen te downloaden als een platte hiërarchie in een map in het ZIP-bestand. Standaard downloadt [!DNL Content Hub] de geselecteerde elementen en hun uitvoeringen in afzonderlijke mappen in een ZIP-bestand.
      ...
      >[!NOTE]
      >
      > * Content Hub slaat uw selectie op (**[!UICONTROL Create a separate folder for each asset]**) als uw voorkeur en behoudt deze voor toekomstige downloads.
...
* [ Slimme gewas ](/help/assets/dynamic-media/image-profiles.md#creating-image-profiles): Het slimme gewas concentreert zich slechts op het essentiële deel van een activa tijdens het bebouwingsproces. Dynamic Media Smart snijdt kunstmatige intelligentie uit, aangedreven door Adobe AI, om het interessepunt bij te houden, zodat onze middelen er optimaal uitzien op alle schermgrootten. [!DNL Adobe Experience Manager] Met Slim uitsnijden worden de breedte en hoogte van een element en de titel weergegeven. Zie meer bij [ gebruikend Slimme Uitsnede met de Dynamische Media van AEM Assets ](https://experienceleague.adobe.com/en/docs/experience-manager-learn/assets/dynamic-media/images/smart-crop-feature-video-use).
  ...
  >[!NOTE]
  > 
  > In het deelvenster Download worden alleen aangepaste statische uitvoeringen weergegeven. De standaardminiaturen van `cq5dam.*` worden niet weergegeven in Content Hub.
...
## Veelgestelde vragen {#faqs-download-assets-content-hub}
...
Ja, kunnen de beheerders **toelaten of onbruikbaar maken toelaten beschikbaarheid van vertoningen** en **download van originele activa** knevels beschikbaar in **Vertoningen** tabel op de Configuratie UI om de download van originele activa te beheren of beschikbaarheid van vertoningen in AEM Assets Content Hub toe te laten.
...
* Stel het doel voor elementgoedkeuring in op Content Hub.

* In de Configuratie UI, ga naar het lusje van Uitvoeringen en laat de beschikbaarheid van vertoningen toe.

* Als het nieuwe programma is toegevoegd aan Dynamic Media met OpenAPI, slaat u bestaande voorinstellingen voor afbeeldingen opnieuw op via de beheerweergave > Gereedschappen > Assets > Voorinstellingen afbeelding (Bewerken > Opslaan).
