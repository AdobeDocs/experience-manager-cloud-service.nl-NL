...
AEM Assets Content Hub is een onderdeel van Adobe Experience Manager Assets as a Cloud Service.
...
## Ik heb AEM Assets Content Hub ingeschakeld in mijn productieprogramma/omgeving. Mag ik het uitschakelen? {#can-i-disable-content-hub}
...
U kunt snel middelen vinden die gebruikend AEM Assets Content Hub op AEM as a Cloud Service milieu door worden geupload:
...
Adobe biedt migratiesupport van Brand Portal naar AEM Assets Content Hub die u kunt gebruiken door een Adobe-ondersteuningsticket te maken.
...
De onderstaande tabel geeft een overzicht van de belangrijkste verschillen tussen AEM Assets Content Hub en Brand Portal:
...
## Hoe kan ik een opslagplaats selecteren om elementen alleen voor de geselecteerde omgeving in AEM Assets Content Hub weer te geven? {#select-repository-multiple-environments}

Wanneer u AEM Assets Content Hub for Production en andere lagere omgevingen voor hetzelfde programma hebt geconfigureerd, kunt u de opslagplaats selecteren en de middelen voor de geselecteerde omgeving weergeven. Voer de volgende stappen uit:
...
## Hoe kan AEM Assets Content Hub de miniatuurvoorvertoning voor ZIP-bestandstype weergeven? {#thumbnail-preview-zip-file}

Als u een miniatuurvoorvertoning wilt weergeven voor bestandstypen zoals .ZIP in AEM Assets Content Hub, kunt u een uitvoering met de naam `cq5dam.preview.jpg` of `cq5dam.preview.png` toevoegen aan de hoofdmap van het pad waar het .ZIP-bestand beschikbaar is in de AEM as a Cloud Service-ontwerpomgeving.
