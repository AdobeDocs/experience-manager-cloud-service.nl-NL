...
In condities of setters kan naar omgevings- of Pipeline-geheimen (stap implementeren) worden verwezen met `${{..}}` syntaxis. Deze kunnen worden gebruikt op plaatsen waar een letterlijke waarde kan worden gebruikt.
...
Hier volgen enkele richtlijnen voor het werken met geheimen:

* De geheimen van het milieu moeten als a [ Cloud Manager worden opgesteld geheim-type milieu variabele ](/help/operations/config-pipeline.md#secret-env-vars). Selecteer Alle voor het veld Toegepaste service.
* Geheime verwijzingen worden niet geïnterpoleerd binnen tekenreeksen (bijvoorbeeld `"Token ${{AUTH_TOKEN}}"` werkt niet)
* Een milieu-geheim waarnaar wordt verwezen, moet niet worden verwijderd als er nog steeds naar wordt verwezen in de configuratie.
...
* Geheimen moeten periodiek worden geroteerd. Er is een sectie op hoe te [ te roteren sleutels ](#rotating-secrets), die goede veiligheidspraktijk is.
