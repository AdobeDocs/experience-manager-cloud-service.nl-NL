...
Alle tags hebben een naamruimte. Als er geen naamruimte is opgegeven, wordt de tag toegewezen aan de standaardnaamruimte, die `TagID` `default`, d.w.z. `/content/cq:tags/default`.  De titel is standaard ingesteld op `Standard Tags`in dergelijke gevallen.
...
Bovendien fungeren containercodes (of supercodes) in een taxonomie als de subsom van alle subcodes: bijvoorbeeld inhoud gelabeld met `fruit/apple` wordt beschouwd als getagd met `fruit` ook, d.w.z. zoeken naar inhoud die alleen is gelabeld met `fruit` zou ook de inhoud vinden waaraan `fruit/apple`.
...
* [Tags in verschillende talen,](tagging-applications.md#tags-in-different-languages) waarin het gebruik van de API&#39;s als ontwikkelaar wordt beschreven
...
Later wordt de pagina bijgewerkt met `cq:tags` de oude referenties automatisch opruimen. Dit wordt geactiveerd omdat het omzetten van een verplaatste tag via de API de doeltag retourneert, waardoor de doeltag-id wordt opgegeven.
