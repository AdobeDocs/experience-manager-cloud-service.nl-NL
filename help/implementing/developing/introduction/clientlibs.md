...
* Clips toegankelijk maken via een pad dat toegankelijk is via de [verzender](/help/implementing/dispatcher/disp-overview.md)
...
>[!TIP]
>
>Ontwikkelaars aan de voorzijde die CSS en Javascript voor AEM projecten creëren zouden zich ook met het [AEM Archetype van het Project en zijn geautomatiseerd front-end bouwstijlproces moeten vertrouwd maken.](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uifrontend.html)
...
Alle JavaScript-, CSS- en andere front-end elementen moeten worden behouden in de module [ui.frontend van het AEM Project Archetype.](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uifrontend.html) Dankzij de flexibiliteit van het archetype kunt u uw moderne webgereedschappen gebruiken om deze bronnen te maken en te beheren.

Het archetype kan de bronnen vervolgens compileren in één CSS- en JS-bestand, en deze automatisch insluiten in een `cq:clientLibraryFolder` opslagplaats.
...
Elke `cq:ClientLibraryFolder` is gevuld met een set JS- en/of CSS-bestanden, samen met enkele ondersteunende bestanden (zie hieronder). Belangrijke eigenschappen van de `cq:ClientLibraryFolder` code zijn als volgt geconfigureerd:

* `allowProxy`: Aangezien alle clientlibs onder moeten worden opgeslagen, verleent dit bezit toegang tot clientlibraries `apps`volmachtsservlet. Zie [Een clientbibliotheekmap zoeken en de onderstaande Servlet](#locating-a-client-library-folder-and-using-the-proxy-client-libraries-servlet) Proxyclientbibliotheken gebruiken.
...
Als de map met clientbibliotheken een of meer bronbestanden bevat die tijdens runtime worden samengevoegd tot één JS- en/of CSS-bestand. De naam van het gegenereerde bestand is de knooppuntnaam met de extensie `.js` of de `.css` bestandsnaam. Het bibliotheekknooppunt met de naam `cq.jquery` resulteert bijvoorbeeld in het gegenereerde bestand met de naam `cq.jquery.js` of `cq.jquery.css`.
...
* Eén `js.txt` bestand en/of één `css.txt` bestand waarmee de bronbestanden worden geïdentificeerd die in de gegenereerde JS- en/of CSS-bestanden moeten worden samengevoegd
...
1. Open CRXDE Lite in een webbrowser (`https://<host>:<port>/crx/de`).
1. Selecteer de `/apps` map en klik op **Maken > Knooppunt** maken.
1. Voer een naam in voor de bibliotheekmap en selecteer **Type** in de lijst `cq:ClientLibraryFolder`. Klik op **OK** en vervolgens op Alles **** opslaan.
...
1. Als u statische bronnen moet beheren, maakt u een submap met de naam `resources` onder de clientbibliotheekmap.
   * Als u statische bronnen in de map opslaat, `resources`kunnen deze niet worden gebruikt voor een publicatie-instantie.
1. Bronbestanden toevoegen aan de bibliotheekmap.
   * Dit wordt typisch gedaan door het front-end bouwstijlproces van het [AEM Archetype van het Project.](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/archetype/uifrontend.html)
...
Nadat de clientbibliotheekmap naar wens is [geconfigureerd,](#creating-clientlib-folders) kunnen uw clientlibs via proxy worden aangevraagd. Als voorbeeld:
...
Met de `allowProxy` eigenschap kunt u het volgende aanvragen:
...
Clientbibliotheken worden geladen via een helpersjabloon die door AEM wordt geboden en die toegankelijk is via `data-sly-use`. De helpermalplaatjes zijn beschikbaar in dit dossier, dat door kan worden geroepen `data-sly-call`.
...
[Raadpleeg de HTML-documentatie](https://docs.adobe.com/content/help/en/experience-manager-htl/using/getting-started/getting-started.html#loading-client-libraries) voor meer informatie over het laden van clientlibs via HTML.
...
De meeste clientlibs zijn vereist voor de AEM-publicatie-instantie. Dat wil zeggen dat de meeste clientlibs-doeleinden de eindgebruikerservaring van de inhoud moeten opleveren. Voor clientlibs op publicatie-instanties kunnen [front-end buildtools](#fed-for-aemaacs) worden gebruikt en geïmplementeerd via [clientbibliotheekmappen, zoals hierboven beschreven.](#creating-clientlib-folders)
...
Als u clientbibliotheken op auteur moet gebruiken, kunt u uw clientbibliotheken maken met `/apps` dezelfde methoden als voor publiceren, maar deze rechtstreeks schrijven in `/apps/.../clientlibs/foo` plaats van een volledig project te maken om het te beheren.
...
Extra instellingen voor de clientbibliotheek kunt u beheren via het deelvenster **Adobe Granite HTML Library Manager** van de systeemconsole `https://<host>:<port>/system/console/configMgr`).
...
* `embed`: Wordt gebruikt om code uit andere bibliotheken in te sluiten. Als knooppunt knooppunten `F` en `G` insluit, is de resulterende HTML een samenvoeging van inhoud van knooppunten `H`en `G` `H`.

### Koppeling naar afhankelijke instellingen {#linking-to-dependencies}

Wanneer de code in de map met clientbibliotheken verwijst naar andere bibliotheken, identificeert u de andere bibliotheken als afhankelijkheden. De `ui:includeClientLib` tag die verwijst naar de clientbibliotheekmap zorgt ervoor dat de HTML-code een koppeling bevat naar het gegenereerde bibliotheekbestand en de afhankelijkheden.
...
De bibliotheek `/etc/clientlibs/myclientlibs/publicmain` is bijvoorbeeld afhankelijk van de `cq.jquery` bibliotheek. De pagina die verwijst naar de hoofdclientbibliotheek genereert HTML met de volgende code:
...
In het voorbeeld in de vorige sectie Code [insluiten van andere bibliotheken](#embedding-code-from-other-libraries) , sluit de `/etc/client/libraries/myclientlibs/publicmain` clientbibliotheekmap de `/apps/myapp/clientlib` clientbibliotheekmap in. Als u de parameter aan de webpagina toevoegt, wordt de volgende koppeling in de broncode van de webpagina gemaakt:
