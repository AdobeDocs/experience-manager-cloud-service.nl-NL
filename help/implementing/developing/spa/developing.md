...
Bij het ontwikkelen van toepassingen voor één pagina op AEM wordt ervan uitgegaan dat de ontwikkelaar aan de voorzijde de beste werkwijzen volgt bij het maken van een SPA. Als u als front-end ontwikkelaar deze algemene beste praktijken evenals weinig AEM-specifieke principes volgt, zal uw SPA met [AEM en zijn inhoud-creatie mogelijkheden](introduction.md#content-editing-experience-with-spa)functioneren.

* **[Draagbaarheid](#portability)** - De onderdelen moeten net als alle andere onderdelen zo draagbaar mogelijk zijn. De SPA moet met draagbare en herbruikbare onderdelen worden gebouwd.
...
Als u AEM ontwerpfuncties niet hoeft te ondersteunen, moet u mogelijk een ander [SPA ontwerpmodel](#spa-design-models)overwegen.
...
Als de [principes van het ontwikkelen van SPA in AEM](#spa-development-principles-for-aem) worden gevolgd, dan zal uw SPA met alle gesteunde AEM tevreden auteurseigenschappen functioneren.
...
<table>
 <tbody>
  ...
  <tr>
   <td>AEM wordt gebruikt als een headless CMS zonder het kader van SDK van de <a href="/help/implementing/developing/spa/reference-materials.md">SPA te gebruiken.</a></td>
...
>[!NOTE]
>
>Hoewel alle modellen in AEM worden ondersteund, kunnen de auteurs van de inhoud, alleen door het derde model te implementeren (en daardoor de aanbevolen [SPA ontwikkelingsbeginselen in AEM](#spa-development-principles-for-aem)te volgen), communiceren met en de inhoud van de SPA bewerken in AEM zoals ze gewend zijn.

## Bestaande SPA migreren naar AEM {#migrating-existing-spas-to-aem}

In het algemeen geldt dat als uw SPA de [SPA ontwikkelingsbeginselen voor AEM](#spa-development-principles-for-aem)volgt, uw SPA in AEM werkt en bewerkbaar is met de AEM SPA Editor.
...
1. **In React componenten, heb toegang tot het model via`this.props.cqModel`**
   ...
   Zodra componenten zijn overeengekomen en het JSON-model is geïmplementeerd, kan de front-end ontwikkelaar de SPA ontwikkelen en eenvoudig toegang krijgen tot het JSON-model via `this.props.cqModel`.
...
Raadpleeg de gids Aan de slag voor uw gekozen framework voor meer informatie over het gebruik `MapTo` en het samenstellen van SPA voor AEM in het algemeen.
...
* **AEM-auteur**
   ...
   1. Wanneer de SPA toepassing wordt uitgevoerd, wordt de JSON opgevraagd, zodat de app het DOM van de pagina met de `cq-data` kenmerken kan samenstellen.
...
* [Aan de slag met SPA in AEM die Reageren](getting-started-react.md) gebruiken toont hoe een basisSPA wordt gebouwd om met de SPA Redacteur in AEM te werken gebruikend Reageren.
* [Aan de slag met SPA in AEM met gebruik van Hoek](getting-started-angular.md) laat zien hoe een elementaire SPA is gemaakt om met de SPA Editor te werken in AEM met gebruik van Hoek.
