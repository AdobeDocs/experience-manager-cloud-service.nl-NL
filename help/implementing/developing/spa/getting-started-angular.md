...
# Aan de slag met SPA in AEM Gebruik van Hoek {#getting-started-with-spas-in-aem-using-angular}
...
Het `package.json` bestand definieert de vereisten van het algemene SPA. De minimum vereiste AEM gebiedsdelen zijn hier vermeld.
...
Het ingangspunt in de SPA is het `app.module.ts` bestand dat hier wordt weergegeven, zodat u zich kunt concentreren op de belangrijke inhoud.
...
De code `MainComponent` voegt de JSON-representatie van het paginamodel in en verwerkt de inhoud om elk element van de pagina in te pakken of te versieren. Meer informatie over het `Page` document vindt u in het document [SPA Blauwdruk](blueprint.md).
